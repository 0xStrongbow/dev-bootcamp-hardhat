...
skip.if(!developmentChains.includes(network.name))
    ...
    it('initial token supply should be a positive value', async () => {
        ...
        console.log(
            "Token Total Supply Value: ",
            new web3.utils.BN(result._hex).toString()
        )
        expect(new web3.utils.BN(result._hex).toString())
    ...
    it('ensure balance goes down after transfer of tokens', async () => {
        ...
        console.log(
            "Current sender token balance: ", currentSenderBalance.toString()
        )
        console.log(
            "Current receiver token balance: ",
        ...
        console.log(
            "New receiver token balance: ", newReceiverBalance.toString()
        )
        // Run the assertion
        expect(newSenderBalance.toString())
        .to.be.a.bignumber.that.is.lessThan(currentSenderBalance.toString())
    })
})
